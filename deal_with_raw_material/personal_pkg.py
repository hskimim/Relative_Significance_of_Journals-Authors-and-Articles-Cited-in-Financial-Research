...
def find_start_page(file_) : 
    ...
    page_ls = [i for i in clean_text(file_).split("\x0c") if i]
...
def find_end_page(file_) : 
    except_page_ls = []
    page_ls = clean_text(file_).split("\x0c")[find_start_page(file_):]
    ls = ['표 1','테이블 1','테이블','테 이 블 1','테 이 블','table 1','Table','Table 1','TABLE 1','TABLE' ,'Appendix','Appendix 1','appendix 1','APPENDIX 1','부록','부 록','Endnotes','Figure 1','FIGURE 1',\
        '표 I','테이블 I','테 이 블 I','table I','Table I','TABLE I' ,'Appendix I','appendix I','APPENDIX I','Endnotes','Figure I','Figure','FIGURE I']
    ...
    if '”' in ','.join(page_ls) :
        try : 
            ...
            return find_start_page(file_) + new_last_page + 1
        except : pass

    if len(except_page_ls) > 0 :
        return find_start_page(file_) + min(except_page_ls) + 1
...
def slicing_the_references(file_) :
    page_ls = [i for i in clean_text(file_).split('\x0c') if i]
    if find_end_page(file_) == -1 :
        return page_ls[find_start_page(file_) : ]
    else : return page_ls[find_start_page(file_) : find_end_page(file_)]

def total_process_to_slicing_the_references(file_):
    page_ls = slicing_the_references(file_)
    ls = ['references','참고 문헌','참 고 문 헌','참  고  문  헌' , '참고문헌','참조문헌','<참 고 문 헌>','reference']
    start_idx = \
[re.search(i,slicing_the_references(file_)[0].lower()).end() for i in ls if i in slicing_the_references(file_)[0].lower()][0]
    page_ls[0] = slicing_the_references(file_)[0][start_idx :]
    return page_ls

def split_sentences(file_,remove_duplicated=True):

    refer = [re.sub('[-]\s[0-9]+\s[-]','',i) for i in total_process_to_slicing_the_references(file_)]
    ...
    for i in range(len(tuning_process1)) :
        split_ls = re.split('[(][a-zA-Z]{,5}20[0-9]{2}[a-zA-Z]*[)][.]|[(][a-zA-Z]{,5}19[0-9]{2}[a-zA-Z]*[)][.]',tuning_process1[i])
        findall_ls = re.findall('[(][a-zA-Z]{,5}20[0-9]{2}[a-zA-Z]*[)][.]|[(][a-zA-Z]{,5}19[0-9]{2}[a-zA-Z]*[)][.]',tuning_process1[i])
        tuning_process2 += \
        [split_ls[idx] + findall_ls[idx] if idx != len(split_ls)-1 else split_ls[-1] for idx in range(len(split_ls))]
