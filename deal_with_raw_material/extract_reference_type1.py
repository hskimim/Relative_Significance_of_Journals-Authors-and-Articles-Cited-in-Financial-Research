...
def final_tuning_new_references(new_references):
    referenecs_new = [ref.split('\n') for ref in new_references]
    references_new = [','.join(ref) for ref in referenecs_new]
    references_new = [ref for ref in referenecs_new if len(ref) <500]
...
def bundles_of_tuning_process(file_):
    '''
    references 를 line by line 으로 잘라주는 함수 셋을 모두 모아둔 번들 함수입니다.
    공백이 두 칸이 있는 것을 split해주는 옵션이 기본적으로 있습니다.
    '''
    #display(Markdown('### It might take some times. please wait'))
    new_references = make_new_references_new_vers(file_,one_more_tuned=True)
    new_references = fine_tuning_the_new_ref(new_references)
    new_references = finer_tuning_new_references(new_references)
    new_references = fine_tuning_the_new_ref(new_references)
    new_references = final_tuning_new_references(new_references)
    new_references = [name for ref in new_references for name in ref]
    return new_references
